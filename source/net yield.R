...
rp=read.csv(paste0('./output intermediate/rp_medians_',file_suffix,'.csv'),header=T,stringsAsFactors = F)%>%
  mutate(rp=rp_median)%>%
  select(region,Year,rp,WEIGHT)

#load calculated vacancy data
vac=read.csv(paste0('./output intermediate/vac_data_',file_suffix,'.csv'),header=T,stringsAsFactors = F)%>%
  mutate(vac_rate=ifelse(is.na(vac_rate),predict_vac_rate,vac_rate))%>%
  select(region,Year,vac_rate)

#load your own tax data
regions=read.csv(paste0('./input data/',file_suffix,'.csv'),header=T,stringsAsFactors=F)
colnames(regions)[1]='region'
tax=regions%>%
  inner_join(read.csv('./input data/taxes.csv',header=T,stringsAsFactors = F),by='state')%>%
...
write.csv(panel,
          file=paste0('./output final/net_yield_',file_suffix,'.csv'),row.names = F)
