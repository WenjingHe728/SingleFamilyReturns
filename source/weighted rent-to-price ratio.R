...
write.csv(rent_own_ratio%>%select(region,Year,decile,rent_own_ratio),
          file=paste0('./output intermediate/rent_own_ratio_',file_suffix,'.csv'),row.names = F)

#save house level rp_owned and weight_nonparam
write.csv(pr%>%select(region,Year,weight_nonparam,rp_owned),
          file=paste0('./output intermediate/rp_owned_',file_suffix,'.csv'),row.names = F)
...
write.csv(pr%>%
            ...
            summarize(rp_median=rp_weighted_median(rp_owned,weight_nonparam),
                      WEIGHT=sum(WEIGHT)),
          file=paste0('./output intermediate/rp_medians_',file_suffix,'.csv'),row.names = F)
